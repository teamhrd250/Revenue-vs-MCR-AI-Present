...
DEFAULT_FILE = os.path.join("data", "SEP_V4_Final_Project_Productivity_Template4.xlsx")
...
def inject_ai_presenter_css():
    st.markdown("""
    <style>
    div[data-testid="stVerticalBlock"]:has(#sep-ai-presenter-marker) {
        position: fixed;
        top: 52px;
        right: 92px;
        z-index: 999999;
    ...
    div[data-testid="stVerticalBlock"]:has(#sep-ai-presenter-marker) button {
...
def render_ai_presenter(ctx: dict):
    ...
    host = st.container()
    with host:
        st.markdown('<span id="sep-ai-presenter-marker"></span>', unsafe_allow_html=True)
