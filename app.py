# BUILD: AI_PRESENTATION_ENGINE_FINAL_2026_07_19
...
def _build_presentation_flow(ctx: dict) -> list[dict]:
    """Create a board-by-board narrative from the active SEP data."""
    m = ctx.get("metrics", {})
    ...
    return [
        {
            "board": BOARD_LABELS[0],
            "title": "Executive Overview",
            "focus": "Company Health, Business Posture, Revenue, MCR, Gross Margin, RPE, dan Capacity Risk",
            "narration": (
                f"Pada {period_label}, Company Health berada pada {health.get('score')}/100 dengan status {health.get('status')}. "
                f"Revenue tercatat {_money(m.get('revenue'), currency)}, MCR {_pct(m.get('mcr'))}, Gross Margin {_pct(m.get('gross_margin'))}, "
                f"dan RPE {_money(m.get('rpe'), currency)}. Business posture adalah {posture}. Fokus Direksi adalah menjaga kualitas margin, "
                "mengendalikan manpower cost, dan memastikan kapasitas delivery tetap aman."
            ),
        },
        {
            "board": BOARD_LABELS[1],
            "title": "Revenue & Productivity",
            "focus": "Revenue growth, RPE, manpower cost growth, gross margin, MCR, dan service-line productivity",
            "narration": (
                f"Revenue berubah {_pct(m.get('revenue_yoy'))}, sementara RPE berubah {_pct(m.get('rpe_yoy'))}. "
                f"Gross Margin berada pada status {margin} dan MCR pada status {mcr_state}. "
                "Manajemen perlu memastikan revenue tumbuh lebih cepat daripada headcount dan manpower cost, serta memprioritaskan service line dengan margin dan revenue per HC terbaik."
            ),
        },
        {
            "board": BOARD_LABELS[2],
            "title": "Workforce Performance",
            "focus": "Headcount, cost per HC, FJA cost mix, workforce portfolio, dan productivity per function",
            "narration": (
                f"Headcount rata-rata tercatat {_num(m.get('headcount'))} dengan perubahan {_pct(m.get('headcount_yoy'))}. "
                f"Cost per HC sebesar {_money(m.get('cost_per_hc'), currency)}. "
                "Arah tindakan adalah menilai fungsi berbiaya tinggi terhadap output, melakukan redistribusi workload dan reskilling, serta membuka hiring hanya untuk bottleneck yang tervalidasi."
            ),
        },
        {
            "board": BOARD_LABELS[3],
            "title": "Capacity & Delivery Risk",
            "focus": "Utilization, overtime, backlog, SLA breach, critical-role dependency, dan Capacity Score",
            "narration": (
                f"Capacity Risk berada pada level {cr.get('state')} dengan skor {cr.get('score')}/100. "
                f"Utilization maksimum {_pct(d.get('max_utilization'))}, overtime {_num(d.get('total_overtime'))} jam, backlog {_num(d.get('total_backlog'))}, "
                f"dan SLA breach {_num(d.get('total_sla_breach'))}. Prioritasnya adalah mengurangi bottleneck melalui workload balancing, process improvement, cross-training, dan selective hiring bila diperlukan."
            ),
        },
        {
            "board": BOARD_LABELS[4],
            "title": "Strategic Decision Center",
            "focus": "Management Attention, Company Health components, hiring decision, MCR guardrail, dan scenario simulator",
            "narration": (
                f"Company Health {health.get('score')}/100 menunjukkan keputusan ekspansi harus diuji terhadap MCR, Gross Margin, RPE, dan revenue gap. "
                "Skenario hanya layak disetujui apabila pipeline revenue dapat dibuktikan, projected MCR tetap dalam guardrail, dan kapasitas tambahan memiliki kebutuhan delivery yang jelas."
            ),
        },
        {
            "board": BOARD_LABELS[5],
            "title": "Benchmark & Methodology",
            "focus": "Current KPI versus framework, gap benchmark, confidence, formula, dan threshold",
            "narration": (
                f"KPI utama saat ini menunjukkan MCR {mcr_state}, margin {margin}, dan capacity risk {cr.get('state')}. "
                "Benchmark digunakan sebagai management guardrail, bukan target otomatis. Setiap gap perlu memiliki baseline internal, owner, target realistis, dan periode pencapaian yang disepakati."
            ),
        },
        {
            "board": BOARD_LABELS[6],
            "title": "CEO Closing Report",
            "focus": "Board conclusion, top decisions, executive owner, dan 30-60-90 day actions",
            "narration": (
                f"Kesimpulan eksekutif menempatkan perusahaan pada status {health.get('status')} dengan skor {health.get('score')}/100. "
                "Prioritas 90 hari adalah revenue recovery, margin protection, serta people-cost dan capacity control. Setiap prioritas perlu executive owner, target numerik, dan review mingguan."
            ),
        },
        {
            "board": BOARD_LABELS[7],
            "title": "Appendix & Data Governance",
            "focus": "Formula, source sheet, analysis mode, data confidence, version control, dan audit trail",
            "narration": (
                f"Dashboard menggunakan mode {ctx.get('analysis_mode')} dengan confidence {ctx.get('period', {}).get('Data_Confidence', '-')}. "
                "Sebelum keputusan final, pastikan sumber data, formula, target, dan asumsi proyeksi telah direkonsiliasi serta disetujui oleh data owner."
            ),
        },
    ]
...
render_presentation_autoplay(presentation_flow, seconds_per_step=18)
