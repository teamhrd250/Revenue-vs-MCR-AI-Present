...
def _historical_year_evaluation(question: str, ctx: dict, currency: str) -> str | None:
    ...
    return (
        ...
        f"**Bukti SEP:** Revenue {_money(row.get('Total_Revenue'), currency)} "
        f"(peringkat {ranks['revenue']['rank']} dari {ranks['revenue']['count']}), RPE {_money(row.get('RPE'), currency)} "
        f"(peringkat {ranks['rpe']['rank']}), Gross Margin {_pct(row.get('Gross_Margin_Pct'))} "
        f"(peringkat {ranks['gross_margin']['rank']}), MCR {_pct(row.get('MCR_Pct'))} "
        f"(peringkat efisiensi {ranks['mcr']['rank']}), Headcount {_num(row.get('Total_Headcount'))}, "
