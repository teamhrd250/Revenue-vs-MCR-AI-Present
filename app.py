...
def _openai_presenter_answer(question: str, ctx: dict, route: dict, api_key: str) -> str:
    ...
    prompt = (
        ...
        "langsung, objektif, dan hanya berdasarkan JSON data. Jangan mengarang angka. Jelaskan implikasi bisnis dan satu rekomendasi. "
...
def executive_management_direction(latest, bs, bp, mq, mh, cr):
    ...
    if posture == "DEFENSIVE" and margin in ["WATCH", "PRESSURE", "CRITICAL"] and mcr in ["COST_PRESSURE", "CRITICAL"] and capacity in ["MEDIUM", "HIGH", "CRITICAL"]:
        return ("Perusahaan perlu memprioritaskan stabilisasi kinerja dengan menjaga kualitas margin, mengendalikan manpower cost, dan memastikan kapasitas operasional tetap memadai. "
                "Penambahan headcount sebaiknya dilakukan secara selektif pada fungsi yang terbukti menjadi bottleneck, sementara perbaikan produktivitas, pricing, direct cost, dan utilisasi sumber daya perlu menjadi fokus utama agar indikator kembali menuju level ideal secara bertahap.")
    if score >= 85 and posture in ["HIGH_LEVERAGE", "BALANCED"] and margin in ["HEALTHY", "STRONG"] and mcr == "HEALTHY" and capacity == "LOW":
        return ("Kondisi perusahaan relatif sehat dan seimbang. Manajemen perlu mempertahankan pertumbuhan yang berkualitas dengan menjaga MCR, gross margin, produktivitas per karyawan, dan capacity risk tetap dalam batas ideal. "
                "Ekspansi dapat dilakukan secara selektif sepanjang didukung revenue pipeline dan business case yang terukur.")
    if (posture == "DEFENSIVE" or (pd.notna(rev_g) and rev_g < 0)) and mcr in ["COST_PRESSURE", "CRITICAL"]:
        return ("Fokuskan upaya pada pemulihan revenue dan pengendalian manpower cost. Hiring non-critical perlu dibatasi, sementara aktivitas komersial, pricing, utilisasi tenaga kerja, dan kontribusi setiap fungsi terhadap revenue perlu ditinjau lebih ketat agar MCR kembali menuju batas ideal.")
    if margin in ["PRESSURE", "CRITICAL"]:
        return ("Prioritaskan perbaikan kualitas revenue melalui evaluasi pricing, COGS, penggunaan subcontractor, scope creep, dan rework. Pertumbuhan revenue sebaiknya tidak hanya dinilai dari nominal, tetapi juga dari margin yang dihasilkan.")
    if capacity in ["HIGH", "CRITICAL"] and mcr in ["HEALTHY", "WATCH", "ULTRA_EFFICIENCY"]:
        return ("MCR yang relatif sehat perlu dijaga, namun tekanan kapasitas harus ditangani agar tidak mengganggu delivery. Tindakan awal diarahkan pada redistribusi workload, optimasi proses, pengendalian overtime, dan selective hiring hanya pada fungsi yang mengalami bottleneck nyata.")
    if (pd.notna(hc_g) and pd.notna(rev_g) and hc_g > rev_g) or (pd.notna(rpe_g) and rpe_g < 0):
        return ("Perusahaan perlu meningkatkan produktivitas tenaga kerja sebelum melakukan ekspansi headcount lebih lanjut. Evaluasi difokuskan pada RPE, utilisasi, output per fungsi, serta kontribusi revenue dan delivery dari setiap penambahan tenaga kerja.")
    return ("Manajemen perlu menjaga keseimbangan antara pertumbuhan revenue, kualitas margin, manpower cost, produktivitas, dan kapasitas delivery. Setiap keputusan ekspansi atau penambahan headcount dilakukan secara selektif berdasarkan bukti bottleneck, revenue pipeline, dan dampaknya terhadap MCR, RPE, serta gross margin.")
...
def revenue_management_direction(latest, mq, mh):
    ...
    mcr = str(mh.get("state", "")).upper()
    if pd.notna(rev_g) and rev_g < 0:
        return "Pemulihan revenue perlu menjadi prioritas melalui penguatan pipeline, review conversion, pricing, dan project mix. Pada saat yang sama, manpower cost dan headcount perlu dikendalikan agar penurunan revenue tidak memperburuk MCR dan produktivitas."
    if margin in ["PRESSURE", "CRITICAL"]:
        return "Prioritaskan perbaikan kualitas revenue melalui evaluasi pricing, COGS, subcontractor, scope creep, dan rework. Service line dengan revenue besar tetapi margin rendah perlu direview sebelum ekspansi kapasitas dilakukan."
    if (pd.notna(hc_g) and pd.notna(rev_g) and hc_g > rev_g) or (pd.notna(cost_g) and pd.notna(rev_g) and cost_g > rev_g) or (pd.notna(rpe_g) and rpe_g < 0):
        return "Pertumbuhan saat ini belum sepenuhnya produktif. Tahan ekspansi kapasitas yang belum memiliki dukungan revenue, tingkatkan RPE, dan arahkan sumber daya ke service line dengan gross margin sehat serta penggunaan manpower yang efisien."
    if pd.notna(rev_g) and rev_g > 0 and pd.notna(rpe_g) and rpe_g > 0 and margin in ["HEALTHY", "STRONG"] and mcr in ["HEALTHY", "WATCH"]:
        return "Pertumbuhan revenue dan produktivitas perlu dipertahankan dengan memastikan kenaikan headcount serta manpower cost tetap lebih rendah daripada pertumbuhan output. Prioritaskan service line dengan margin tinggi dan kontribusi revenue yang konsisten."
    return "Fokuskan pengelolaan revenue pada pertumbuhan yang berkualitas: revenue harus tumbuh lebih cepat daripada manpower cost, RPE perlu membaik, dan service line prioritas harus memenuhi guardrail gross margin serta MCR."
...
def workforce_management_direction(latest, dept, fja_latest):
    ...
    if fja_latest is not None and not fja_latest.empty and "Cost_Share" in fja_latest.columns:
        top_cost_share = pd.to_numeric(fja_latest["Cost_Share"], errors="coerce").max()
    if (pd.notna(hc_g) and pd.notna(rev_g) and hc_g > rev_g) or (pd.notna(rpe_g) and rpe_g < 0):
        return "Perusahaan perlu meningkatkan produktivitas tenaga kerja sebelum melakukan ekspansi headcount lebih lanjut. Evaluasi difokuskan pada RPE, utilisasi, output per fungsi, serta kontribusi revenue dan delivery dari setiap penambahan tenaga kerja."
    if pd.notna(top_cost_share) and top_cost_share >= 0.45:
        return "Konsentrasi manpower cost pada kelompok fungsi terbesar perlu ditinjau untuk memastikan outputnya sebanding dengan biaya. Prioritaskan redistribusi workload, simplifikasi proses, penguatan capability, dan selective hiring hanya pada kebutuhan yang tidak dapat ditutup secara internal."
    return "Jaga keseimbangan antara Revenue Generator, Revenue Enabler, Support, dan Governance. Fungsi dengan headcount atau manpower cost tinggi perlu memiliki output, kontribusi delivery, atau perlindungan risiko yang terukur sebelum penambahan kapasitas disetujui."
...
def capacity_management_direction(cr):
    ...
    if state in ["HIGH", "CRITICAL"]:
        return "Tekanan kapasitas perlu ditangani melalui redistribusi workload, pengendalian overtime, percepatan penyelesaian backlog, dan pengurangan ketergantungan pada critical role. Selective hiring hanya dilakukan pada fungsi yang menunjukkan bottleneck konsisten dan berdampak langsung terhadap delivery atau SLA."
    if state == "MEDIUM":
        return "Kapasitas operasional perlu dijaga agar tidak berkembang menjadi risiko delivery. Lakukan review per fungsi atas utilisasi, overtime, backlog, dan SLA; selesaikan akar masalah melalui perbaikan proses dan redistribusi workload sebelum mempertimbangkan selective hiring."
    if pd.notna(util) and util < 0.75 and (pd.isna(overtime) or overtime <= 100) and (pd.isna(backlog) or backlog <= 0) and (pd.isna(sla) or sla <= 0):
        return "Kapasitas operasional masih relatif terkendali. Pertahankan utilisasi pada rentang optimal dan hindari penambahan headcount sebelum terdapat bukti peningkatan workload, backlog, atau risiko SLA."
    return "Pertahankan pemantauan kapasitas secara berkala dan pastikan utilisasi, overtime, backlog, SLA, turnover, serta critical dependency tetap dalam guardrail. Gunakan perbaikan proses sebagai tindakan awal sebelum ekspansi kapasitas permanen."
...
def strategic_management_direction(scenario_mcr, gross_margin_sim, revenue_gap, targets):
    ...
    if pd.notna(scenario_mcr) and scenario_mcr <= mcr_healthy and gross_margin_sim >= gm_healthy and pd.notna(revenue_gap) and revenue_gap <= 0:
        return "Skenario ekspansi dapat dipertimbangkan karena projected MCR dan gross margin masih berada dalam batas target serta kebutuhan revenue telah memiliki buffer. Persetujuan tetap diberikan secara bertahap dan dikaitkan dengan realisasi revenue serta kebutuhan delivery aktual."
    if pd.notna(scenario_mcr) and scenario_mcr <= mcr_watch and gross_margin_sim >= gm_healthy:
        return "Skenario masih dapat dipertimbangkan secara terbatas, namun berada pada zona watch. Direksi perlu mensyaratkan revenue pipeline yang terukur, milestone pencapaian, dan evaluasi ulang MCR serta RPE sebelum penambahan kapasitas berikutnya."
    return "Keputusan penambahan headcount perlu ditahan atau dikaji ulang apabila projected MCR melewati guardrail, gross margin belum memadai, atau revenue gap masih positif. Uji kembali kombinasi revenue target, biaya per HC, dan jumlah tambahan HC sebelum persetujuan diberikan."


def benchmark_management_direction(latest, mq, mh, cr):
    gaps = []
    if _safe_float(latest.get("Revenue_YoY")) < 0.15: gaps.append("Revenue Growth")
    if _safe_float(latest.get("RPE_YoY")) < 0.15: gaps.append("RPE Growth")
    ...
    if gaps:
        return f"Prioritaskan kalibrasi dan perbaikan pada indikator yang belum memenuhi framework, terutama {', '.join(gaps)}. Benchmark digunakan sebagai alarm dan batas diskusi, sedangkan target akhir tetap disesuaikan dengan project mix, struktur biaya, historical performance, dan risiko operasional Starcom."
    return "Kerangka target saat ini masih relevan sebagai management guardrail. Pertahankan review berkala agar threshold tetap sesuai dengan perubahan model bisnis, pricing, struktur biaya, dan tingkat kematangan data."


def ceo_management_direction(latest, bs, bp, mq, mh, cr):
    return executive_management_direction(latest, bs, bp, mq, mh, cr) + " Tetapkan owner, milestone, dan jadwal review agar dampak keputusan terhadap revenue, MCR, RPE, gross margin, dan capacity score dapat dipantau pada siklus berikutnya."


def appendix_management_direction(period, mode):
    confidence = str(period.get("Data_Confidence", "Unknown")).upper()
    if confidence in ["LOW", "UNKNOWN"]:
        return "Data dengan confidence rendah atau sumber yang belum lengkap perlu dikonfirmasi sebelum digunakan sebagai dasar keputusan Direksi. Pastikan rekonsiliasi sumber, ownership, formula, dan periode selesai terlebih dahulu."
    if mode in ["Annual Projection", "MCR Focus"]:
        return "Hasil pada mode proyeksi atau stress-test harus diperlakukan sebagai skenario, bukan realisasi. Setiap keputusan perlu mencantumkan asumsi, sumber data, formula, owner, dan batas penggunaan agar hasil dashboard tetap dapat diaudit."
    return "Jaga konsistensi nama sheet, kolom, periode, formula, dan ownership data. Setiap perubahan target atau metodologi perlu terdokumentasi dan disetujui melalui governance sebelum digunakan dalam keputusan manajemen."
