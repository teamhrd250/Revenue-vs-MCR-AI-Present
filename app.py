# BUILD: AI_EXECUTIVE_ADVISOR_GOVERNED_CONTEXT_2026_07_20
...
def _historical_year_evaluation(question: str, ctx: dict, currency: str) -> str | None:
    """Evaluate whether a requested year is ideal by comparing its KPI mix against SEP history."""
    q = " ".join((question or "").lower().split())
    decision_terms = ["ideal", "terbaik", "lebih baik", "optimal", "paling baik", "paling ideal", "mana yang baik"]
    if not any(term in q for term in decision_terms):
        return None
    years = [int(y) for y in re.findall(r"\b(?:19|20)\d{2}\b", question or "")]
    if not years:
        return None
    requested = years[0]
    history = ctx.get("historical_comparison", []) or []
    rows = [r for r in history if r.get("Year") is not None]
    hist_map = {int(r.get("Year")): r for r in rows}
    target = hist_map.get(requested)
    if not target:
        return f"Data tahun {requested} belum tersedia pada histori SEP, sehingga status ideal belum dapat dinilai."
    ...
    revs = sorted([f(r.get("Total_Revenue")) for r in rows if pd.notna(f(r.get("Total_Revenue")))])
    rpes = sorted([f(r.get("RPE")) for r in rows if pd.notna(f(r.get("RPE")))])

    def percentile_score(value, values, weight):
    ...
    def gm_score(gm):
        if pd.isna(gm): return 0.0
        if gm >= 0.30: return 20.0
        if gm >= 0.20: return 17.0
        if gm >= 0.15: return 12.0
        if gm >= 0.10: return 7.0
        return 2.0

    def mcr_score(mcr):
        if pd.isna(mcr): return 0.0
        if 0.05 <= mcr <= 0.07: return 20.0
        if 0.07 < mcr <= 0.09: return 16.0
        if mcr < 0.05: return 14.0
        if 0.09 < mcr <= 0.14: return 8.0
        return 2.0

    def score_row(r):
        rev = f(r.get("Total_Revenue")); rpe = f(r.get("RPE"))
        gm = f(r.get("Gross_Margin_Pct")); mcr = f(r.get("MCR_Pct"))
        rev_g = f(r.get("Revenue_YoY")); rpe_g = f(r.get("RPE_YoY")); hc_g = f(r.get("Headcount_YoY"))
        growth = 0.0
        if pd.notna(rev_g) and rev_g > 0: growth += 8.0
        if pd.notna(rpe_g) and rpe_g > 0: growth += 6.0
        if pd.notna(rev_g) and pd.notna(hc_g) and rev_g > hc_g: growth += 6.0
        return round(percentile_score(rev, revs, 20) + percentile_score(rpe, rpes, 20) + gm_score(gm) + mcr_score(mcr) + growth, 1)

    ranking = sorted([(int(r.get("Year")), score_row(r)) for r in rows], key=lambda x: x[1], reverse=True)
    rank = next((i + 1 for i, (y, _) in enumerate(ranking) if y == requested), None)
    score = score_row(target)
    best_year, best_score = ranking[0] if ranking else (requested, score)
    mcr = f(target.get("MCR_Pct")); gm = f(target.get("Gross_Margin_Pct"))
    rev_g = f(target.get("Revenue_YoY")); rpe_g = f(target.get("RPE_YoY")); hc_g = f(target.get("Headcount_YoY"))

    strengths=[]; cautions=[]
    if 0.05 <= mcr <= 0.07: strengths.append("MCR berada pada zona sehat 5–7%")
    elif mcr < 0.05: cautions.append("MCR sangat rendah; efisiensi perlu diuji terhadap kecukupan kapasitas")
    elif mcr <= 0.09: cautions.append("MCR masih zona watch, belum ideal")
    else: cautions.append("MCR berada di atas zona ideal")
    if gm >= 0.20: strengths.append("gross margin berada pada zona sehat")
    elif gm >= 0.15: cautions.append("gross margin masih zona watch")
    else: cautions.append("gross margin berada di bawah zona sehat")
    if pd.notna(rev_g) and rev_g > 0: strengths.append("revenue tumbuh")
    elif pd.notna(rev_g): cautions.append("revenue tidak tumbuh")
    if pd.notna(rpe_g) and rpe_g > 0: strengths.append("RPE meningkat")
    elif pd.notna(rpe_g): cautions.append("RPE menurun")
    if pd.notna(rev_g) and pd.notna(hc_g) and rev_g > hc_g: strengths.append("revenue tumbuh lebih cepat daripada headcount")
    elif pd.notna(rev_g) and pd.notna(hc_g): cautions.append("headcount tumbuh lebih cepat daripada revenue")

    verdict = "IDEAL" if score >= 80 and rank == 1 else "BAIK" if score >= 65 else "BELUM IDEAL"
    strength_text = "; ".join(strengths) if strengths else "belum ada kekuatan utama yang cukup terukur"
    caution_text = "; ".join(cautions) if cautions else "tidak ada peringatan utama dari KPI yang tersedia"
    return (
        f"Kesimpulan: FY {requested} dinilai **{verdict}** dengan skor evaluasi historis {score:.1f}/100 "
        f"dan peringkat {rank} dari {len(ranking)} tahun yang tersedia. "
        f"Revenue {_money(target.get('Total_Revenue'), currency)}, RPE {_money(target.get('RPE'), currency)}, "
        f"MCR {_pct(target.get('MCR_Pct'))}, Gross Margin {_pct(target.get('Gross_Margin_Pct'))}, "
        f"Headcount {_num(target.get('Total_Headcount'))}, dan Manpower Cost {_money(target.get('Total_Manpower_Cost'), currency)}. "
        f"Kekuatan: {strength_text}. Perhatian: {caution_text}. "
        f"Tahun dengan kombinasi KPI terbaik pada data SEP adalah FY {best_year} dengan skor {best_score:.1f}/100. "
        "Penilaian ideal tidak ditentukan oleh revenue tertinggi saja, melainkan kombinasi revenue, RPE, gross margin, MCR, "
        "serta kualitas pertumbuhan dibanding headcount."
    )
...
def _local_presenter_answer(question: str, ctx: dict, route: dict) -> str:
    ...
    # Local fallback when API key is unavailable.
    evaluation = _historical_year_evaluation(question, ctx, currency)
    if evaluation:
        return evaluation
    # Explicit years in the question take precedence over the dashboard's active year.
    years = [int(y) for y in re.findall(r"\b(?:19|20)\d{2}\b", question or "")]
    if len(years) == 1:
        history = ctx.get("historical_comparison", []) or []
        hist_map = {int(r.get("Year")): r for r in history if r.get("Year") is not None}
        ...
        if r:
            if focus == "revenue":
                return (
                    f"Revenue pada FY {y} adalah {_money(r.get('Total_Revenue'), currency)} "
                    f"dengan pertumbuhan {_pct(r.get('Revenue_YoY'))}. "
                    f"Pada tahun yang sama, RPE {_money(r.get('RPE'), currency)}, "
                    f"headcount {_num(r.get('Total_Headcount'))}, MCR {_pct(r.get('MCR_Pct'))}, "
                    f"dan Gross Margin {_pct(r.get('Gross_Margin_Pct'))}."
                )
            return (
                f"Data FY {y} berdasarkan SEP: revenue {_money(r.get('Total_Revenue'), currency)}, "
                f"headcount {_num(r.get('Total_Headcount'))}, RPE {_money(r.get('RPE'), currency)}, "
                f"MCR {_pct(r.get('MCR_Pct'))}, Gross Margin {_pct(r.get('Gross_Margin_Pct'))}, "
                f"dan manpower cost {_money(r.get('Total_Manpower_Cost'), currency)}."
            )
        return f"Data tahun {y} belum tersedia pada historical_comparison SEP."

    if len(years) >= 2:
        history = ctx.get("historical_comparison", []) or []
        hist_map = {int(r.get("Year")): r for r in history if r.get("Year") is not None}
        y1, y2 = years[0], years[1]
        r1, r2 = hist_map.get(y1), hist_map.get(y2)
        if r1 and r2:
            def money(v):
            ...
            def delta(a, b):
                try:
                    if a is None or b in (None, 0):
                        return "-"
                    return _pct((float(a) / float(b)) - 1)
                except Exception:
                    return "-"
            return (
                f"Perbandingan FY {y1} dan FY {y2} berdasarkan data SEP: "
                f"Revenue {y1} {money(r1.get('Total_Revenue'))} vs {y2} {money(r2.get('Total_Revenue'))} "
                f"(perubahan {delta(r1.get('Total_Revenue'), r2.get('Total_Revenue'))}); "
                f"Headcount {_num(r1.get('Total_Headcount'))} vs {_num(r2.get('Total_Headcount'))}; "
                f"RPE {money(r1.get('RPE'))} vs {money(r2.get('RPE'))}; "
                f"MCR {pct(r1.get('MCR_Pct'))} vs {pct(r2.get('MCR_Pct'))}; dan "
                f"Gross Margin {pct(r1.get('Gross_Margin_Pct'))} vs {pct(r2.get('Gross_Margin_Pct'))}. "
                "Pencapaian yang lebih baik ditentukan oleh kombinasi revenue dan RPE yang lebih tinggi, "
                "MCR yang lebih terkendali, serta gross margin yang tetap sehat."
            )
        return (
            f"Data perbandingan tahun {y1} dan {y2} belum lengkap pada historical_comparison SEP. "
            "Pilih rentang tahun yang mencakup kedua periode atau lengkapi data Excel."
        )
...
def _openai_presenter_answer(
    ...
    prompt = (
        ...
        "Pisahkan fakta, interpretasi, dan rekomendasi. Bila data belum cukup, nyatakan data apa yang perlu divalidasi. "
        ...
        f"Pertanyaan terbaru: {question}\nData SEP terpilih: {json.dumps(selected_ctx, ensure_ascii=False, default=str)}"
    )
    try:
        r = requests.post(
