...
def render_board_guidance(title: str, how_items: list[str], summary: str, implication: str, tone: str = "info"):
    tone_class = {
        "success": "board-summary-success",
        "warning": "board-summary-warning",
        "error": "board-summary-error",
        "info": "board-summary-info",
    }.get(tone, "board-summary-info")
    items = "".join(f"<li>{html.escape(str(x))}</li>" for x in how_items)
    st.markdown("### Cara Membaca & Ringkasan Dashboard")
    c1, c2 = st.columns([1.0, 1.15])
    with c1:
        st.markdown(
            f"""
            <div class="board-how-card">
                <div class="board-card-title">ⓘ How to Read — {html.escape(title)}</div>
                <ul>{items}</ul>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with c2:
        st.markdown(
            f"""
            <div class="board-summary-card {tone_class}">
                <div class="board-card-title">Executive Summary</div>
                <div class="board-summary-text">{summary}</div>
                <div class="board-implication"><b>Implikasi Manajemen:</b> {implication}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
with boards[0]:
    ...
    render_business_question("Bagaimana kondisi kesehatan perusahaan saat ini, apa isu paling kritis, dan keputusan apa yang perlu diambil Direksi?", "Menentukan apakah perusahaan siap tumbuh, perlu optimasi, atau harus masuk defensive mode.")
    ...
    render_board_guidance(
        ...
        f"Prioritas Direksi adalah {html.escape(action_title)}. Fokuskan tindakan pada indikator berstatus warning/error dan hindari general hiring tanpa bukti bottleneck.",
...
with boards[1]:
    ...
    render_business_question("Apakah revenue tumbuh bersama produktivitas, bukan hanya karena tambahan headcount dan biaya?", "Menilai kualitas pertumbuhan revenue, RPE, margin, dan efisiensi manpower.")
    c1, c2 = st.columns(2)
    with c1:
    ...
    render_board_guidance(
        ...
        "Validasi apakah pertumbuhan berasal dari produktivitas dan margin, bukan hanya tambahan HC. Prioritaskan service line dengan margin sehat dan penggunaan manpower efisien.",
        _summary_tone(bs, mq, mh, cr),
    )
...
with boards[2]:
    ...
    render_business_question("Apakah struktur SDM sudah optimal dan setiap fungsi memiliki kontribusi yang sebanding dengan biaya serta kapasitasnya?", "Menentukan fungsi yang perlu dioptimalkan, dipertahankan, atau mendapat tambahan kapasitas.")
    fja = data["fja"].copy()
    ...
    render_board_guidance(
        ...
        "Gunakan data ini untuk redistribusi workload, simplifikasi proses, dan selective hiring. Jangan menilai fungsi support hanya dari revenue langsung.",
...
with boards[3]:
    ...
    render_business_question("Apakah organisasi mulai overload dan apakah tekanan delivery memerlukan hiring, redistribusi workload, atau perbaikan proses?", "Mencegah overload, SLA breach, backlog, dan ketergantungan pada critical role.")
    ...
    render_board_guidance(
        ...
        f"Tindak lanjut yang direkomendasikan adalah {html.escape(str(cr.get('recommendation')))}. Validasi akar masalah sebelum menambah HC permanen.",
...
with boards[4]:
    ...
    render_business_question("Keputusan apa yang harus diambil untuk menjaga pertumbuhan tanpa menekan margin, MCR, dan kapasitas delivery?", "Mengubah data menjadi keputusan hiring, cost control, margin recovery, dan prioritas 90 hari.")
    ...
    render_board_guidance(
        ...
        "Direksi perlu memilih tindakan yang memperbaiki indikator terlemah tanpa merusak indikator yang masih sehat. Gunakan simulator untuk menguji konsekuensi sebelum menyetujui hiring.",
        _summary_tone(bs, mq, mh, cr),
    )
...
with boards[5]:
    ...
    render_business_question("Apakah target KPI realistis, dapat dipertanggungjawabkan, dan masih relevan terhadap kondisi perusahaan?", "Memastikan threshold digunakan sebagai guardrail manajemen dan direview secara berkala.")
    ...
    render_board_guidance(
        ...
        "Gunakan benchmark sebagai alarm dan batas diskusi. Keputusan akhir tetap harus mempertimbangkan pipeline, project mix, cash flow, dan risiko operasional Starcom.",
        _summary_tone(bs, mq, mh, cr),
    )
...
with boards[6]:
    ...
    render_business_question("Apa kesimpulan akhir, tiga keputusan utama, dan tindakan berikutnya yang perlu dikawal Direksi?", "Menyatukan seluruh insight menjadi arahan eksekutif dan agenda tindak lanjut.")
    ...
    render_board_guidance(
        ...
        f"Tetapkan owner dan deadline untuk prioritas: {html.escape(action_title)}. Review dampaknya pada MCR, RPE, margin, dan capacity score pada siklus berikutnya.",
        _summary_tone(bs, mq, mh, cr),
    )
...
with boards[7]:
    ...
    render_business_question("Dari mana angka berasal, bagaimana KPI dihitung, dan apakah hasil dashboard dapat diaudit?", "Memastikan transparansi formula, sumber data, ownership, dan governance.")
    st.markdown("""
    ### Core Formula
    ...
    render_board_guidance(
        ...
        "Jaga konsistensi nama sheet, kolom, periode, dan ownership data. Setiap perubahan formula atau target perlu mendapat persetujuan governance.",
        "info",
    )
