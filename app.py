        ...
        with st.popover("🎙 AI Presenter", help="Presentasikan dashboard atau buka Ask Mode", use_container_width=False):
            ...
            with presentation_tab:
                ...
                if st.session_state.get("presentation_active"):
                    ...
                    step = steps[idx]
                    st.markdown(
                        f"<div class='sep-ai-focus'><b>{html.escape(step['title'])}</b><br>"
                        f"<small>{html.escape(step['focus'])}</small></div>",
                        unsafe_allow_html=True,
                    )
                    st.write(step["narration"])
                    st.progress((idx + 1) / len(steps), text=f"{idx + 1} / {len(steps)}")
                    c1, c2, c3, c4 = st.columns(4)
                    if c1.button("⏮", use_container_width=True, key="presentation_prev"):
                        _move_presentation(presentation_flow, -1); st.rerun()
                    pause_label = "▶ Resume" if st.session_state.get("presentation_paused") else "⏸ Pause"
                    if c2.button(pause_label, use_container_width=True, key="presentation_pause"):
                        st.session_state.presentation_paused = not st.session_state.get("presentation_paused")
                        st.session_state.presentation_last_tick = time.time(); st.rerun()
                    if c3.button("⏭", use_container_width=True, key="presentation_next"):
                        _move_presentation(presentation_flow, 1); st.rerun()
                    if c4.button("⏹", use_container_width=True, key="presentation_stop"):
                        st.session_state.presentation_active = False
                        st.session_state.presentation_paused = False
                        st.session_state.presentation_audio = None
                        st.rerun()
                    if st.session_state.get("presentation_sound"):
                        if st.session_state.get("presentation_audio_index") != idx:
                            audio_bytes, audio_err = _synthesize_speech(step["narration"], api_key)
                            st.session_state.presentation_audio = audio_bytes
                            st.session_state.presentation_audio_index = idx
                            if audio_err: st.caption(audio_err)
