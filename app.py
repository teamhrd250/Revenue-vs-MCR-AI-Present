...
def render_presentation_stage(flow: list[dict]):
    ...
    voice_html = ""
    if _VOICE_AVAILABLE:
        voice_html = render_voice_button(narration, key_suffix=f"step_{idx}")

    st.markdown(
        f"""
        <div class="sep-presentation-stage">
          <div class="sep-presentation-badge">🎙️ AI EXECUTIVE PRESENTATION · STEP {idx + 1}/{len(steps)}</div>
          ...
          <div class="sep-presentation-narration">{html.escape(narration)}</div>
          {voice_html}
        </div>
        """,
...
def _synthesize_speech(text: str, api_key: str) -> tuple[bytes | None, str]:
    # Priority 1: OpenAI TTS (if API key available)
    if api_key:
        try:
            r = requests.post(
            ...
            # Fall through to Edge-TTS on OpenAI failure
            pass

    # Priority 2: Edge-TTS (free, no key required) — Bahasa Indonesia voice
    if _VOICE_AVAILABLE:
        try:
            import importlib
            vengine = importlib.import_module("voice_engine")
            audio_b64 = vengine.get_audio_base64(text)
            if audio_b64:
                # Decode base64 data URI to bytes
                import base64
                prefix = "data:audio/mp3;base64,"
                if audio_b64.startswith(prefix):
                    raw_b64 = audio_b64[len(prefix):]
                    return base64.b64decode(raw_b64), ""
                return None, ""
        except Exception as e:
            return None, f"Edge-TTS gagal: {e}"

    return None, "Tidak ada TTS engine tersedia (install edge-tts atau set OPENAI_API_KEY)."
...
def render_ai_presenter(ctx: dict, presentation_flow: list[dict]):
    ...
    if "ai_answer_mode" not in st.session_state:
        st.session_state.ai_answer_mode = "Executive Analysis"
    if "ai_answer_depth_basis" not in st.session_state:
        st.session_state.ai_answer_depth_basis = {
            "mode": "Executive Analysis", "label": "Eksekutif", "reason": "Mode otomatis aktif."
        }
    if "ai_focus_active" not in st.session_state:
        st.session_state.ai_focus_active = False
    if "ai_analysis_basis" not in st.session_state:
    ...
    if "ai_focus_context" not in st.session_state:
        st.session_state.ai_focus_context = ctx

    api_key = _get_openai_api_key()
    host = st.container(key="ai_presenter_float")
    with host:
        with st.popover("🎙 AI Presenter", help="Presentasikan dashboard atau ajukan pertanyaan", use_container_width=False):
            ...
            with presentation_tab:
                ...
                if st.session_state.get("presentation_active"):
                    ...
                    # Voice quick-play button (Edge-TTS)
                    if _VOICE_AVAILABLE and not st.session_state.get("presentation_sound"):
                        voice_html = render_voice_button(step["narration"], key_suffix=f"pop_{idx}")
                        if voice_html:
                    ...
                    if c1.button("⏮", use_container_width=True, key="presentation_prev"):
                        _move_presentation(presentation_flow, -1)
                        st.rerun()
            ...
            with ask_tab:
                st.caption("Satu AI Executive Advisor dengan intelligent routing: fakta SEP, analisis bisnis, dan benchmark dipilih otomatis sesuai pertanyaan.")
                st.caption(
                    "Cakupan sumber dan kedalaman jawaban dipilih otomatis berdasarkan maksud pertanyaan. "
                    "Angka internal tetap hanya berasal dari SEP."
                )
                ...
                if st.session_state.ai_presenter_answer:
                    ...
                    st.caption(
                        f"Dasar analisis: {basis.get('label', 'SEP Internal Data')} · "
                        f"{basis.get('reason', '')}"
                    )
                    depth = st.session_state.get("ai_answer_depth_basis", {})
                    st.caption(
                        f"Kedalaman otomatis: {depth.get('label', 'Eksekutif')} · "
                        f"{depth.get('reason', 'Mode otomatis aktif.')}"
                    )

                if st.session_state.get("ai_conversation"):
                    with st.expander("Riwayat percakapan", expanded=False):
                        for item in st.session_state.ai_conversation[-8:]:
                            label = "Anda" if item.get("role") == "user" else "AI Executive Advisor"
                            st.markdown(f"**{label}:** {item.get('content', '')}")
                ...
                if st.session_state.ai_presenter_answer:
                    ...
                    _render_presenter_visual(route, st.session_state.get("ai_focus_context", ctx), key_scope="popover")
...
def inject_header_feature_css():
    ...
    .sep-exec-highlight h3 {
        color: #93C5FD !important;
        font-size: 14px;
        letter-spacing: 0.08em;
        text-transform: uppercase;
    }
    .sep-exec-highlight .big-number {
        font-size: 3.5rem;
        font-weight: 900;
    ...
    /* ===== RESPONSIVE ===== */
    @media (max-width: 768px) {
        .sep-feature-band { grid-template-columns: 1fr; }
        .sep-presentation-stage { padding: 14px 16px !important; }
    }
    </style>
    """, unsafe_allow_html=True)
