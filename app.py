...
def render_ai_dashboard_focus(ctx: dict):
    """Render an enlarged, board-level visual focus after an Ask Mode question."""
    if not st.session_state.get("ai_focus_active"):
        return
    ...
    with left:
        st.markdown("<div class='sep-dashboard-pointer'>➤</div>", unsafe_allow_html=True)
    with right:
        st.markdown(
            f"<div class='sep-dashboard-focus-head'><div class='sep-dashboard-focus-kicker'>AI VISUAL FOCUS</div>"
            f"<div class='sep-dashboard-focus-title'>{html.escape(route.get('board', 'Dashboard'))}</div>"
            f"<div class='sep-dashboard-focus-sub'>{html.escape(', '.join(route.get('targets', [])))}</div></div>",
            unsafe_allow_html=True,
        )
        focus_ctx = st.session_state.get("ai_focus_context", ctx)
        _render_presenter_visual(route, focus_ctx, key_scope="dashboard_focus")
        if answer:
            with st.expander("Lihat analisis AI", expanded=True):
                st.markdown(answer)
        if st.button("✕ Tutup Fokus Visual", key="close_ai_focus"):
...
render_ai_dashboard_focus(ai_presenter_context)
